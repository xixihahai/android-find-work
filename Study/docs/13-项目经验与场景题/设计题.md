...
### 一句话理解
> 设计题 = 需求分析 + 架构设计 + 核心实现 + 扩展性考虑。从产品视角出发，用技术思维解决。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| 图片加载框架 | 三级缓存(内存/磁盘/网络)+生命周期感知+Bitmap复用+采样压缩 | ★★★★★ |
| 网络请求框架 | 拦截器链(责任链模式)+连接池+缓存+重试+线程调度 | ★★★★★ |
| EventBus设计 | 注册/反注册+事件分发(反射/APT)+线程切换+粘性事件 | ★★★★☆ |
| 路由框架 | APT编译时收集路由表+运行时查表跳转+拦截器+降级策略 | ★★★★★ |
| 日志系统 | 分级过滤+异步写入(mmap)+日志压缩+加密上传+磁盘管理 | ★★★★☆ |
| 埋点系统 | 自动埋点(AOP/ASM)+手动埋点+数据聚合+批量上报+离线缓存 | ★★★★☆ |
| 回答套路 | 先说需求(功能+非功能)→再画架构图→再讲核心模块实现→最后说扩展性 | ★★★★★ |

### 与其他知识的串联
- **图片加载框架 → Glide原理(三级缓存+生命周期)**：理解Glide源码就能回答图片框架设计题
- **网络框架 → OkHttp(拦截器链+连接池)**：OkHttp的拦截器链是责任链模式的经典应用
- **路由框架 → ARouter(APT+组件化)**：APT编译时生成路由映射表，运行时通过path查找目标Activity/Fragment
