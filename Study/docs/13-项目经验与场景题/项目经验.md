...
### 一句话理解
> 项目经验面试 = STAR法则——Situation(什么背景)→Task(什么任务)→Action(你做了什么)→Result(量化结果)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| STAR法则 | 情境→任务→行动→结果，结构化描述项目经验的黄金框架 | ★★★★★ |
| 量化结果 | 必须有数字：启动耗时降低XX%、崩溃率从X‰降到X‰、内存峰值降低XXM | ★★★★★ |
| 技术亮点提炼 | 深度>广度，挑1-2个技术点深入讲原理+实现+效果 | ★★★★★ |
| 技术选型思考 | 对比至少2种方案，说清楚选择理由(性能/维护性/团队熟悉度) | ★★★★☆ |
| 踩坑与解决 | 描述问题现象→排查过程→根因→解决方案，展示问题解决能力 | ★★★★★ |
| 架构演进 | 展示从发现问题→分析→设计→实施→验证的完整思考过程 | ★★★★☆ |
| 避坑要点 | 不要流水账、不要只说用了什么框架、不要没有数据支撑 | ★★★★★ |

### 与其他知识的串联
- **贯穿所有技术模块**——性能优化(启动/渲染/内存)+架构设计(MVVM/组件化)+稳定性(Crash/ANR治理)
- **启动优化项目 → 启动流程(Application.onCreate)+懒加载+异步初始化+启动框架**
- **内存优化项目 → 内存泄漏排查(LeakCanary)+Bitmap优化+OOM治理**
- **架构升级项目 → 组件化(ARouter)+MVVM(ViewModel+LiveData/Flow)+Clean Architecture**
