...
### 一句话理解
> 系统设计面试 = STAR-T法则——在STAR基础上加 Tradeoff(技术取舍)，展示你的架构决策能力。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| STAR-T法则 | Scenario(场景)→Target(目标)→Architecture(架构)→Realization(实现)→Tradeoff(权衡) | ★★★★★ |
| App架构设计 | 壳工程+业务模块+基础模块，模块间通过路由/接口通信(面向接口编程) | ★★★★★ |
| 性能监控体系 | Choreographer帧率+BlockCanary卡顿+LeakCanary泄漏+ANR监控+Crash上报 | ★★★★★ |
| 灰度发布 | 配置中心+用户分群+AB测试+灰度比例控制+数据对比+快速回滚 | ★★★★☆ |
| 动态化方案 | 插件化(加载APK)+热修复(修复Bug)+RN/Flutter(跨平台)+动态配置(配置下发) | ★★★★☆ |
| 架构分层 | 表现层(MVVM)→领域层(UseCase)→数据层(Repository)→基础层(网络/存储) | ★★★★★ |
| 技术权衡 | 任何方案都有取舍：性能vs开发效率、一致性vs可用性、简单vs灵活 | ★★★★★ |

### 与其他知识的串联
- **App架构 → 组件化+MVVM+Clean Architecture**：组件化解耦模块，MVVM分离UI逻辑，Clean Architecture分层职责清晰
- **性能监控 → Choreographer帧率监控+LeakCanary+ANR检测**：Choreographer.postFrameCallback计算帧间隔判断卡顿
- **灰度发布 → 配置中心+AB测试**：通过配置中心下发灰度策略，AB测试对比新旧版本核心指标
