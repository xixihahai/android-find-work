...
### 一句话理解
> 内存泄漏就像忘关水龙头——对象该被回收了，但还有引用指着它(GC Root可达)，垃圾回收器无法回收，内存持续增长直到 OOM。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| 泄漏本质 | 长生命周期对象持有短生命周期对象引用，导致GC Root可达无法回收 | ★★★★★ |
| Handler泄漏 | 非静态内部类Handler持有Activity引用，消息未处理完Activity就销毁了 | ★★★★★ |
| 单例泄漏 | 单例持有Activity Context，Activity销毁但单例还在→用Application Context | ★★★★★ |
| 匿名内部类泄漏 | 匿名Runnable/Listener持有外部类引用，回调时外部类已销毁 | ★★★★☆ |
| 静态变量泄漏 | static集合/变量持有Activity等对象引用，进程不死引用不释放 | ★★★★☆ |
| LeakCanary原理 | 监听Activity销毁→弱引用+ReferenceQueue→触发GC→仍可达则泄漏→dump hprof | ★★★★★ |
| 修复套路 | 改用静态内部类+弱引用；改用Application Context；及时注销/取消 | ★★★★★ |

### 与其他知识的串联
- **Handler泄漏 → Handler机制**：非静态内部类Handler隐式持有外部Activity引用，改用静态内部类+WeakReference
- **LeakCanary → Activity Lifecycle监听+弱引用+GC触发**：利用Application.registerActivityLifecycleCallbacks监听onDestroy
- **静态变量泄漏 → Context体系**：区分Activity Context和Application Context，单例/工具类用Application Context
