...
### 一句话理解
> HashMap 就像一个快递柜——先用 hashCode 找到柜号(数组下标)，遇到冲突(hash碰撞)就挂链表，链表太长(≥8)就变红黑树加速查找。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| HashMap结构 | 数组+链表+红黑树(JDK8)，默认容量16，负载因子0.75 | ★★★★★ |
| hash碰撞解决 | 链表法，链表长度≥8且数组≥64时转红黑树 | ★★★★★ |
| 扩容机制 | 2倍扩容，rehash时高低位拆分(巧妙利用2的幂) | ★★★★☆ |
| ConcurrentHashMap | JDK8用CAS+synchronized(Node级别锁)，废弃分段锁 | ★★★★★ |
| ArrayList vs LinkedList | 数组(随机访问O(1)) vs 双向链表(插入删除O(1)) | ★★★★☆ |
| TreeMap | 红黑树实现，有序，O(logN)操作 | ★★★☆☆ |
| Android优化集合 | ArrayMap(小数据量替代HashMap)、SparseArray(key为int) | ★★★★☆ |

### 与其他知识的串联
- **HashMap → ConcurrentHashMap → 线程安全**：从HashMap线程不安全到ConcurrentHashMap的并发设计，串联多线程同步机制
- **LRU算法 → LruCache**：LinkedHashMap实现LRU，Glide/OkHttp内存缓存的基础，串联图片加载和网络框架
- **Android特有集合 → 内存优化**：ArrayMap/SparseArray避免装箱和额外Entry对象，串联Android性能优化
