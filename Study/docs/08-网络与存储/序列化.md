...
### 一句话理解
> 序列化就像打包行李——Serializable(自动打包但慢)、Parcelable(手动打包但快，Android专用)、JSON(通用格式)、ProtoBuf(极致压缩)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| Serializable | Java原生，反射实现，有大量临时对象+IO开销，适合持久化/网络传输 | ★★★★☆ |
| Parcelable | Android专属，直接内存读写无反射，速度比Serializable快10倍+，适合IPC | ★★★★★ |
| serialVersionUID | 序列化版本号，不一致会反序列化失败(InvalidClassException) | ★★★★☆ |
| JSON方案选型 | Gson(简单)、Moshi(Kotlin友好)、kotlinx.serialization(编译时生成无反射) | ★★★★☆ |
| ProtoBuf | Google开源，二进制格式，体积比JSON小3-10倍，解析快20-100倍 | ★★★★☆ |
| Bundle大小限制 | Binder事务缓冲区约1MB(所有事务共享)，Intent传大数据会TransactionTooLargeException | ★★★★★ |
| Parcelable vs Serializable | 性能选Parcelable(IPC)，简便选Serializable(持久化)，@Parcelize最省事 | ★★★★★ |

### 与其他知识的串联
- **Parcelable → Intent/Bundle传输(四大组件通信)**：Activity跳转传对象必须实现Parcelable/Serializable
- **Bundle大小限制 → Binder缓冲区1MB**：超过限制用ViewModel共享、写文件/数据库、ContentProvider
- **JSON → Gson/Moshi/kotlinx.serialization选型**：新项目推荐kotlinx.serialization(编译时生成，性能好)
- **ProtoBuf → gRPC(后端通信)**：ProtoBuf是gRPC的默认序列化方案，适合高性能后端通信
