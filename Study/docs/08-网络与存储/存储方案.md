...
### 一句话理解
> 存储方案选择——小配置用 DataStore/MMKV(替代SP)，结构化数据用 Room(替代SQLite)，文件用 Storage Access Framework。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| SP问题 | 全量写入XML+commit同步阻塞+apply的onStop等待→ANR高发 | ★★★★★ |
| MMKV | 基于mmap内存映射+protobuf编码，写入即落盘，性能比SP快100倍+ | ★★★★★ |
| DataStore | Jetpack出品，基于协程+Flow，类型安全(Proto DataStore)，替代SP | ★★★★☆ |
| Room | SQLite的ORM封架，编译时APT生成代码，支持Flow/LiveData响应式查询 | ★★★★★ |
| 分区存储 | Android 10+ Scoped Storage，App只能访问自己目录+MediaStore+SAF | ★★★★★ |
| ContentProvider | 跨进程数据共享的标准方案，底层基于Binder(query/insert/update/delete) | ★★★★☆ |
| 存储选型 | KV配置→MMKV/DataStore；关系数据→Room；大文件→文件存储+SAF | ★★★★★ |

### 与其他知识的串联
- **SP ANR问题 → apply是异步但Activity.onStop会等待写入完成**：QueuedWork.waitToFinish()在onStop同步等待所有apply完成
- **MMKV → mmap内存映射(操作系统原理)**：mmap将文件映射到进程虚拟地址空间，写内存即写文件，崩溃也不丢数据
- **Room → 编译时APT生成代码(注解处理器)**：@Dao/@Entity/@Database注解在编译期生成实现类，无运行时反射开销
- **分区存储 → Android 10+ Scoped Storage**：requestLegacyExternalStorage只是过渡方案，Android 11+必须适配
