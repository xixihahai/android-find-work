...
Activity 的核心设计目标是：**管理用户界面的生命周期和用户交互**。

**关键设计约束**：
1. **生命周期管理**：系统可以随时回收和恢复 Activity
2. **用户交互**：处理触摸、按键等用户输入
3. **界面管理**：管理 View 层级和窗口属性
4. **进程隔离**：不同 Activity 可以在不同进程运行
...
**核心设计思想**：
- **生命周期驱动**：系统通过回调通知 Activity 状态变化
- **任务栈管理**：Activity 按启动顺序压入/弹出栈
- **进程通信**：通过 Binder 与系统服务通信
- **状态保存**：异常情况下保存和恢复状态
...
**设计思路**：
Activity 的生命周期是**状态驱动**的设计模式，系统通过回调通知 Activity 状态变化，Activity 根据状态执行相应操作。

**核心设计原则**：
1. **可预测性**：生命周期回调顺序固定，便于开发者理解
2. **可恢复性**：异常情况下可以保存和恢复状态
3. **资源管理**：及时释放不再需要的资源
4. **用户体验**：保证界面切换的流畅性
...
| 回调方法 | 调用时机 | 设计目的 | 典型操作 |
|---------|---------|---------|---------|
| onCreate() | Activity 首次创建 | 初始化基础组件 | 初始化 UI、绑定数据、恢复状态 |
| onStart() | Activity 变为可见 | 准备用户可见界面 | 注册广播、初始化 UI 更新 |
| onResume() | Activity 获得焦点 | 准备用户交互 | 开始动画、获取相机等独占资源 |
| onPause() | Activity 失去焦点 | 暂停当前 Activity | 暂停动画、保存关键数据 |
| onStop() | Activity 完全不可见 | 释放系统资源 | 释放资源、注销广播 |
| onDestroy() | Activity 被销毁 | 清理所有资源 | 释放所有资源、取消网络请求 |
| onRestart() | 从 onStop 恢复 | 重新初始化 | 重新初始化在 onStop 中释放的资源 |

**为什么这样设计**：
- **onPause() 先于 onStop()**：确保关键数据在 Activity 完全不可见前保存
- **onStop() 先于 onDestroy()**：给系统回收内存的机会
- **onCreate() 只调用一次**：避免重复初始化
