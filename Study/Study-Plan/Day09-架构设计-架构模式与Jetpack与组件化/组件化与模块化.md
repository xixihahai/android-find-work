...
### 一句话理解
> 组件化就像乐高积木——每个模块(用户/商品/订单)独立开发运行，通过路由(ARouter)通信，用依赖注入(Hilt)管理依赖。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| 模块化vs组件化 | 模块化=拆分代码,组件化=模块化+独立运行+路由通信 | ★★★★★ |
| ARouter原理 | 编译时APT扫描@Route注解生成路由表,运行时查表跳转 | ★★★★★ |
| 组件间通信 | 路由跳转(ARouter)、接口下沉(base模块)、事件总线 | ★★★★☆ |
| Hilt/Dagger | 编译时生成依赖注入代码,Hilt是Dagger的Android封装 | ★★★★☆ |
| 组件独立调试 | application/library动态切换,sourceSets配置独立manifest | ★★★☆☆ |
| Gradle依赖管理 | api(传递)/implementation(不传递)/compileOnly(仅编译) | ★★★★☆ |

### 与其他知识的串联
- **ARouter → APT注解处理器**：ARouter编译时用APT生成路由表，串联泛型与反射中的注解处理技术
- **Hilt → Dagger依赖注入**：Hilt简化了Dagger在Android中的使用，自动绑定Application/Activity/Fragment等组件
- **组件间通信 → 接口下沉到base模块**：通过公共模块定义接口、业务模块实现接口、ServiceLoader/SPI机制发现实现
...
│  │        ↓               ↓                                       │   │
│  │  ┌───────────┐  ┌───────────┐                                  │   │
│  │  │ Activity  │  │ViewModel  │  @ViewModelScoped               │   │
│  │  │ Component │  │ Component │                                  │   │
│  │  │@Activity  │  └───────────┘                                  │   │
│  │  │ Scoped    │                                                 │   │
│  │  └─────┬─────┘                                                 │   │
