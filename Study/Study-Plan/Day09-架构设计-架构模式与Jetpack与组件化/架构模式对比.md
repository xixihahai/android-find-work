...
### 一句话理解
> 架构模式演进就像公司管理进化——MVC(老板什么都管)→MVP(老板雇了经理)→MVVM(经理自动化报告)→MVI(用流水线标准化流程)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| MVC | Activity同时充当View和Controller，耦合严重 | ★★★★☆ |
| MVP | Presenter持有View接口，解耦但接口爆炸 | ★★★★☆ |
| MVVM | ViewModel+数据绑定，自动更新UI，Google官方推荐 | ★★★★★ |
| MVI | 单向数据流(Intent→Model→View)，状态可预测 | ★★★★★ |
| Clean Architecture | 依赖规则:外层依赖内层，内层不知道外层(洋葱模型) | ★★★★☆ |
| 架构选型 | 简单页面MVVM足够,复杂交互MVI更可控 | ★★★★☆ |

### 与其他知识的串联
- **MVVM → ViewModel+LiveData/Flow**：MVVM的VM层就是Jetpack ViewModel，数据通知用LiveData或StateFlow
- **MVI → Compose+单向数据流**：Compose天然适合MVI，UI = f(State)，配合sealed class定义状态和意图
- **Clean Architecture → 组件化分层基础**：Domain层纯Kotlin无Android依赖，可跨模块复用，是组件化的分层指导思想
...
│  │  │  ┌─────────────────────────────────────────────────┐    │   │   │
│  │  │  │  Data Layer (Repository实现, DataSource, API)    │    │   │   │
│  │  │  │  ┌─────────────────────────────────────────┐    │    │   │   │
│  │  │  │  │         Domain Layer (核心)              │    │    │   │   │
