...
### 一句话理解
> 网络请求就像寄快递——HTTP是快递单格式，TCP是快递通道(三次握手=确认地址)，TLS是加密包装，OkHttp是快递公司(拦截器=各个处理环节)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| TCP三次握手 | SYN→SYN+ACK→ACK，确保双方收发能力正常(两次不够：服务端无法确认客户端收到) | ★★★★★ |
| TCP四次挥手 | FIN→ACK→FIN→ACK，双方各自关闭发送通道(TIME_WAIT等2MSL防止丢包) | ★★★★★ |
| HTTPS/TLS | 非对称加密协商密钥→对称加密传数据；TLS1.3只需1-RTT(0-RTT恢复) | ★★★★★ |
| OkHttp拦截器链 | Retry→Bridge→Cache→Connect→CallServer，责任链模式依次处理 | ★★★★★ |
| 连接池 | OkHttp ConnectionPool默认5个空闲连接存活5分钟，复用避免重复握手 | ★★★★★ |
| DNS优化 | HTTPDNS绕过LocalDNS，避免DNS劫持/解析慢，配合缓存+预解析 | ★★★★☆ |
| 弱网优化 | 超时分级+重试策略+数据压缩+请求优先级+离线缓存 | ★★★★☆ |

### 与其他知识的串联
- **TCP三次握手 → 连接池复用(OkHttp ConnectionPool)**：连接池复用已建立的TCP连接，省去握手开销
- **HTTPS → TLS1.3优化(减少RTT)**：TLS1.3将握手从2-RTT减到1-RTT，Session恢复可0-RTT
- **DNS优化 → HTTPDNS(避免DNS劫持)**：HTTPDNS直接向权威DNS发HTTP请求，绕过运营商LocalDNS
- **弱网优化 → 重试策略(Retrofit+OkHttp拦截器)**：通过自定义Interceptor实现智能重试、降级策略
