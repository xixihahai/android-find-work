...
### 一句话理解
> 内存优化核心三件事——省(减少不必要的内存分配)、放(及时释放不用的资源)、复(对象池复用减少GC)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| Bitmap内存计算 | 宽×高×每像素字节数(ARGB_8888=4字节)，注意density缩放 | ★★★★★ |
| inSampleSize | 采样压缩，值为2则宽高各缩一半，内存变1/4 | ★★★★★ |
| inBitmap复用 | 复用已有Bitmap内存，避免反复分配/回收(Android 4.4+大小可不同) | ★★★★★ |
| LruCache | 基于LinkedHashMap的最近最少使用缓存，超容量淘汰最久未用的 | ★★★★★ |
| 大图加载 | BitmapRegionDecoder局部加载+手势缩放，不用把整张图加载到内存 | ★★★★☆ |
| 对象池 | Message.obtain/Pools.acquire复用对象，减少GC压力 | ★★★★☆ |
| 内存抖动 | 短时间大量创建/销毁对象→频繁GC→卡顿(循环中创建对象是典型原因) | ★★★★★ |

### 与其他知识的串联
- **Bitmap优化 → inBitmap复用(RecyclerView图片加载)**：Glide内部就使用inBitmap复用BitmapPool中的Bitmap
- **LruCache → 三方库缓存(Glide/OkHttp)**：Glide内存缓存用LruCache，OkHttp磁盘缓存用DiskLruCache
- **对象池 → Message.obtain(Handler机制)**：Handler机制中Message通过享元模式复用，避免频繁new Message
- **大图加载 → BitmapRegionDecoder**：结合自定义View+Matrix实现超大图片的分块加载与手势操作
