...
### 一句话理解
> 稳定性优化三板斧——Java Crash(try-catch+全局捕获)、Native Crash(Signal Handler)、ANR(Input 5s/前台广播10s/前台Service 20s超时)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| Java Crash | 未捕获异常→UncaughtExceptionHandler→进程退出 | ★★★★★ |
| Native Crash | 信号机制(SIGSEGV/SIGABRT),用breakpad/coffeecatch捕获 | ★★★★☆ |
| ANR触发条件 | Input 5s、BroadcastReceiver 10s(前台)、Service 20s(前台) | ★★★★★ |
| ANR分析 | traces.txt堆栈+/data/anr/目录+Systrace分析主线程阻塞 | ★★★★★ |
| 全局Crash捕获 | Thread.setDefaultUncaughtExceptionHandler设置全局处理器 | ★★★★☆ |
| OOM治理 | 内存泄漏(LeakCanary检测)+大对象优化+图片压缩 | ★★★★★ |
| 线上监控 | Bugly/Sentry/Firebase Crashlytics集成上报 | ★★★★☆ |

### 与其他知识的串联
- **ANR → Handler机制**：ANR本质是主线程Looper消息处理超时，理解Handler消息机制才能定位ANR根因
- **Native Crash → NDK/JNI**：Native层崩溃通过Linux信号机制捕获，breakpad生成minidump文件，需addr2line还原堆栈
- **线上监控 → Bugly/Sentry集成**：完整的稳定性体系需要Crash上报+堆栈符号化+报警+归因分析的闭环
