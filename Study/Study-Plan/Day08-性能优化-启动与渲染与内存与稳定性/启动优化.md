...
### 一句话理解
> 启动优化就像优化早高峰通勤——减少必经之路(主线程任务)、开多条路并行(异步初始化)、提前出发(预加载)、走捷径(Baseline Profile)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| 冷/温/热启动 | 冷(进程不存在)→温(进程在Activity销毁)→热(Activity在后台) | ★★★★★ |
| 启动耗时测量 | adb shell am start -W / Displayed时间 / Systrace分析 | ★★★★★ |
| 异步初始化 | 有向无环图(DAG)拓扑排序,处理SDK初始化依赖关系 | ★★★★★ |
| 延迟初始化 | IdleHandler(空闲执行)、按需加载(首次使用时初始化) | ★★★★☆ |
| Baseline Profile | 预编译热点代码为机器码,减少JIT编译时间 | ★★★★☆ |
| App Startup | 替代ContentProvider初始化,减少启动开销 | ★★★★☆ |
| 启动白屏优化 | 设置windowBackground主题,SplashScreen API(Android 12+) | ★★★☆☆ |

### 与其他知识的串联
- **Application.onCreate → ContentProvider更早执行**：Jetpack App Startup利用ContentProvider时机合并初始化，串联四大组件启动顺序
- **冷启动流程 → Zygote fork + ActivityThread.main**：理解系统启动流程才能找到优化切入点，串联Android Framework知识
- **IdleHandler → Handler机制应用**：MessageQueue空闲时执行延迟任务，是Handler消息机制的重要应用场景
...
│  │  16. → ApplicationThread.scheduleTransaction() (Binder IPC)     │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│       │                                                                 │
│       ↓ Binder IPC                                                      │
│  ┌─────────────────────────────────────────────────────────────────┐   │
 ...
 * Displayed 时间包含:
 * - 进程创建时间 (冷启动时)
 * - Application 初始化时间 (冷启动时)
 * - Activity 创建时间
 * - 布局加载时间
 * - 首帧绘制时间
 * 
 * 即从 startActivity 调用到首帧绘制完成的完整时间
 */
