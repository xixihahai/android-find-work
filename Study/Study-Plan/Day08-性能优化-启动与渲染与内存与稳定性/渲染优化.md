...
### 一句话理解
> 渲染优化核心一句话——保证每帧16.6ms内完成 measure+layout+draw，否则就掉帧卡顿。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| 16.6ms原则 | 60fps=每帧16.6ms,超时就掉帧,用户感知卡顿 | ★★★★★ |
| 渲染流程 | Input→Animation→Measure→Layout→Draw→Sync→GPU渲染 | ★★★★★ |
| 过度绘制 | 同一像素被绘制多次,用GPU呈现模式分析(蓝绿红) | ★★★★☆ |
| 布局优化 | ConstraintLayout打平层级、merge/ViewStub减少嵌套 | ★★★★★ |
| RecyclerView优化 | ViewHolder复用、DiffUtil差量更新、预加载、共享缓存池 | ★★★★★ |
| Choreographer | VSYNC信号调度器,doFrame回调驱动每帧渲染 | ★★★★☆ |
| 离屏渲染 | 硬件加速(RenderThread)、Canvas.saveLayer开销大 | ★★★☆☆ |

### 与其他知识的串联
- **Choreographer → 渲染机制核心调度器**：Choreographer接收VSYNC信号触发doFrame，按Input→Animation→Traversal顺序回调
- **布局层级 → ConstraintLayout原理**：ConstraintLayout用约束求解替代多层嵌套，一次measure/layout完成(减少measure次数)
- **RecyclerView → 列表性能重灾区**：缓存四级(Scrap→Cache→ViewCacheExtension→RecycledViewPool)，串联缓存设计思想
