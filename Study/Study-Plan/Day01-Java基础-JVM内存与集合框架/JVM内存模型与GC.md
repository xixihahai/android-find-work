...
### 一句话理解
> JVM 内存就像一栋办公楼——方法区是档案室(存类信息)、堆是开放工位(存对象)、栈是独立办公室(每个线程一间)、程序计数器是你手上的待办清单。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| 内存区域划分 | 堆(对象)、栈(线程私有)、方法区(类信息)、PC(指令地址) | ★★★★★ |
| 堆的分代 | 新生代(Eden+S0+S1) + 老年代，默认比例 8:1:1 | ★★★★★ |
| GC算法 | 标记-清除(碎片)、复制(浪费空间)、标记-整理(慢但无碎片) | ★★★★☆ |
| 垃圾收集器 | CMS(低延迟)、G1(大堆首选)、ZGC(超低延迟) | ★★★★☆ |
| 四种引用 | 强→软(内存不足回收)→弱(GC即回收)→虚(跟踪回收) | ★★★★★ |
| GC Roots | 栈引用、静态变量、JNI引用、同步锁对象 | ★★★★☆ |
| 对象创建过程 | 类加载→分配内存→初始化零值→设置对象头→执行init | ★★★☆☆ |

### 与其他知识的串联
- **GC → Android内存管理**：Android ART 的 GC 机制(CC收集器)是 JVM GC 的变种，面试常考 ART vs Dalvik 的 GC 差异
- **内存区域 → OOM类型**：堆溢出(对象太多)、栈溢出(递归太深)、方法区溢出(动态代理生成太多类)，对应不同排查思路
- **四种引用 → 框架缓存设计**：Glide 用弱引用管理活跃资源、软引用做内存缓存；LeakCanary 用弱引用检测泄漏
