...
### 一句话理解
> 类加载就像入职流程——先查档(加载class文件)→核实身份(验证)→办工牌(准备,分配内存)→签合同(解析符号引用)→入职培训(初始化静态变量)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| 类加载五阶段 | 加载→验证→准备→解析→初始化(记忆口诀:加验准解初) | ★★★★★ |
| 双亲委派模型 | 先让父加载器加载,父加载不了再自己加载,保证核心类安全 | ★★★★★ |
| 打破双亲委派 | SPI机制(线程上下文类加载器)、OSGi、热修复/插件化 | ★★★★☆ |
| Android ClassLoader | PathClassLoader(已安装App) vs DexClassLoader(动态加载) | ★★★★★ |
| 类初始化时机 | new/static字段/反射/子类触发父类/main方法等(6种主动引用) | ★★★★☆ |
| 准备vs初始化 | 准备阶段赋零值(int=0),初始化阶段赋真正的值 | ★★★☆☆ |

### 与其他知识的串联
- **双亲委派 → 插件化/热修复**：插件化通过自定义ClassLoader打破双亲委派加载插件类，热修复将补丁dex插入dexElements数组最前面
- **Android ClassLoader → 四大组件加载**：PathClassLoader加载App类，DexClassLoader用于插件化加载外部dex/apk
- **类加载 → 反射**：反射的Class.forName()会触发类初始化，ClassLoader.loadClass()则不会
