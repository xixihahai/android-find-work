...
### 一句话理解
> synchronized 就像卫生间门锁——一次只能一人用。volatile 就像公告栏——写了所有人立刻能看到。Lock 就像排号系统——更灵活但更复杂。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| synchronized原理 | 对象头Mark Word+Monitor，编译为monitorenter/monitorexit | ★★★★★ |
| 锁升级 | 无锁→偏向锁(单线程)→轻量级锁(CAS自旋)→重量级锁(阻塞) | ★★★★★ |
| volatile | 保证可见性+禁止重排序,不保证原子性(内存屏障实现) | ★★★★★ |
| AQS框架 | state+CLH队列，ReentrantLock/CountDownLatch/Semaphore的基础 | ★★★★☆ |
| CAS | Compare And Swap，乐观锁思想，ABA问题用版本号解决 | ★★★★☆ |
| ThreadLocal | 线程本地变量,每个线程独立副本(ThreadLocalMap) | ★★★★☆ |
| 线程池 | 核心线程→队列→最大线程→拒绝策略(记忆:核队最拒) | ★★★★★ |

### 与其他知识的串联
- **synchronized锁升级 → JVM实现**：偏向锁/轻量级锁是JVM优化，与对象头Mark Word结构直接关联
- **AQS → 并发工具类**：ReentrantLock、CountDownLatch、Semaphore、ReadWriteLock都基于AQS，理解AQS等于理解所有
- **volatile → 单例双重检查锁**：DCL单例必须加volatile防止指令重排导致拿到未初始化的对象
