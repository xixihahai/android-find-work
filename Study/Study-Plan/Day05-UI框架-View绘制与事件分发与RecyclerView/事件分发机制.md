# Android 事件分发机制：核心设计与实现

## 速记总结

### 一句话理解
> 事件分发就像**公司审批流程**：CEO(Activity)把文件交给总监(ViewGroup)，总监可以自己批(拦截)也可以交给员工(View)处理，如果员工处理不了就逐级上报。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| 三个核心方法 | dispatch(分发)→intercept(拦截,仅ViewGroup)→onTouch(消费) | ★★★★★ |
| 传递方向 | 分发：从外向内(Activity→View)；回传：从内向外(View→Activity) | ★★★★★ |
| 事件序列一致性 | DOWN 谁消费，后续 MOVE/UP 就归谁 | ★★★★★ |
| 滑动冲突 | 外部拦截法(父View决定) vs 内部拦截法(子View决定) | ★★★★☆ |
| OnTouchListener优先级 | OnTouchListener.onTouch > onTouchEvent > OnClickListener | ★★★★☆ |

### 与其他知识的串联
```
事件分发与其他知识的关系
├── RecyclerView: 滚动处理基于事件分发，嵌套滑动用 NestedScrolling 扩展
├── View绘制: 事件分发确定"谁响应触摸"，View绘制确定"怎么显示"
├── Handler: 事件从 InputManagerService 通过 Handler 发送到主线程
├── Activity生命周期: onPause 时可能收到 ACTION_CANCEL
└── WMS: InputManagerService 通过 WMS 确定事件应该发给哪个窗口
```

### 面试答题公式
> **事件分发题万能公式**：先说传递链路(Activity→ViewGroup→View) → 再说三个方法的关系 → 最后说实际场景(滑动冲突怎么解决)
...
事件分发机制的核心设计目标是：**将用户的触摸事件高效、准确地传递给目标 View，并支持复杂的交互场景**。

**关键设计约束**：
1. **准确性**：事件必须传递给正确的 View（触摸点所在的 View）
2. **灵活性**：支持自定义拦截逻辑，处理滑动冲突
3. **性能**：高效的事件分发，避免不必要的遍历
4. **一致性**：保证事件序列的完整性（DOWN-MOVE-UP）
...
- **模板方法模式**：dispatchTouchEvent 定义分发流程，子类可重写特定步骤
- **状态机**：事件序列构成状态机，DOWN 开始，UP/CANCEL 结束
- **坐标转换**：父 View 坐标系 → 子 View 坐标系
...
**核心规则**：
1. **一致性**：如果 View 消费了 DOWN，后续 MOVE/UP 都会交给它
2. **完整性**：事件序列必须完整，不能中途丢失
3. **可取消**：父 View 拦截时会发送 CANCEL 给子 View
