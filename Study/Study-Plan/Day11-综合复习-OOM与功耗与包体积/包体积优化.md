...
### 一句话理解
> 包体积优化就像收拾行李——删掉不用的(代码/资源shrink)、压缩能压的(WebP/R8)、按需打包(App Bundle)、大件寄快递(So动态下发)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| APK组成 | DEX(30%)+Resources(40%)+Assets(10%)+So(15%)+其他(5%) | ★★★★★ |
| R8/ProGuard | 代码混淆+无用代码移除+优化,R8是ProGuard升级版(更快更强) | ★★★★★ |
| 资源优化 | 图片转WebP、移除无用资源(shrinkResources)、资源混淆(AndResGuard) | ★★★★★ |
| So优化 | 只保留armeabi-v7a/arm64-v8a、动态下发大So库 | ★★★★☆ |
| App Bundle | Google Play按设备配置分发(语言/密度/ABI),体积减少15%+ | ★★★★☆ |
| DEX优化 | 移除无用代码(Tree Shaking)、减少方法数、D8编译器优化 | ★★★★☆ |
| 插件化下发 | 功能模块按需下载,不打进主包,减少初始安装体积 | ★★★☆☆ |

### 与其他知识的串联
- **R8 → ProGuard升级版**：R8整合了脱糖+编译+混淆+优化+shrink，一步完成(替代原来D8+ProGuard两步)
- **So动态下发 → 网络+类加载**：下载So文件后通过System.loadLibrary加载，涉及网络请求、文件存储和类加载机制
- **App Bundle → Google Play分发**：AAB格式配合Dynamic Feature Module实现按需下载，串联组件化/模块化设计
...
│  │  ✓ 降低 CDN 带宽成本                                            │   │
│  │  ✓ 提升应用商店排名                                             │   │
│  │  ✓ 满足应用商店上架要求 (Google Play APK 100MB/AAB 200MB 限制)  │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
