...
### 一句话理解
> 功耗优化核心原则——能不醒就不醒(减少唤醒)、醒了赶紧干完(批量任务)、干完赶紧睡(释放WakeLock)。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| Doze模式 | 屏灭+静止+未充电→系统进入深度休眠，限制网络/Alarm/JobScheduler | ★★★★★ |
| App Standby | 按使用频率分桶(Active→Rare)，桶越低限制越多(JobScheduler/网络/Alarm) | ★★★★☆ |
| WakeLock | 持有WakeLock防止CPU休眠，必须释放否则电量杀手(用超时机制兜底) | ★★★★★ |
| AlarmManager | Doze下setExactAndAllowWhileIdle可突破限制，但频率也受限 | ★★★★☆ |
| WorkManager | 首选后台任务方案，内部按API选择JobScheduler(23+)/AlarmManager+BroadcastReceiver | ★★★★★ |
| Battery Historian | Google官方电量分析工具，可视化展示WakeLock/Alarm/网络/CPU时间线 | ★★★★☆ |
| 后台限制(8.0+) | 后台不能startService，需startForegroundService并在创建后5s内调startForeground | ★★★★★ |

### 与其他知识的串联
- **Doze模式 → AlarmManager受限(用WorkManager替代)**：Doze下普通Alarm被延迟到维护窗口，WorkManager能感知Doze约束
- **WakeLock → 必须释放(否则电量杀手)**：用try-finally或acquire(timeout)确保释放，Battery Historian可看到未释放的WakeLock
- **后台限制 → Service(8.0+不能后台启动普通Service)**：后台启动前台Service需用startForegroundService+通知栏
- **JobScheduler → WorkManager的底层实现之一**：WorkManager在API 23+使用JobScheduler，低版本fallback到AlarmManager
