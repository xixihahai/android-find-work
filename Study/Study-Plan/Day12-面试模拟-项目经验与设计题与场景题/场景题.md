...
### 一句话理解
> 场景题 = 问题定义 → 排查工具 → 根因分析 → 解决方案 → 量化验证。不是答"我知道什么"而是答"我怎么解决"。

### 核心知识点速记
| 知识点 | 一句话记忆 | 面试频率 |
|--------|-----------|---------|
| 列表优化 | RecyclerView四级缓存+DiffUtil差量更新+预加载+ViewType优化+图片优化 | ★★★★★ |
| OOM排查 | Memory Profiler看趋势→dump hprof→MAT/AS分析→找大对象/泄漏链→修复验证 | ★★★★★ |
| ANR排查 | /data/anr/traces.txt看主线程堆栈→Systrace看锁/IO→Looper Printer打印耗时消息 | ★★★★★ |
| 启动优化 | 异步初始化(有向无环图)+延迟加载+ContentProvider合并+闪屏页优化 | ★★★★★ |
| 内存泄漏排查 | LeakCanary自动发现→GC Root引用链定位→弱引用/生命周期修复 | ★★★★★ |
| Crash分析 | 符号化堆栈→分类聚合→Top Crash优先修→灰度验证→崩溃率监控 | ★★★★☆ |
| 卡顿优化 | Systrace定位→主线程耗时操作迁移→布局层级优化→过度绘制消除 | ★★★★★ |

### 与其他知识的串联
- **列表优化 → RecyclerView(缓存+DiffUtil+布局优化)**：四级缓存(Scrap→Cache→ViewCacheExtension→RecycledViewPool)
- **OOM排查 → Memory Profiler+MAT+LeakCanary**：Profiler实时看内存趋势，MAT分析dominator tree找大对象
- **ANR排查 → traces.txt+Systrace+Looper Printer**：traces.txt看主线程阻塞在哪，Systrace看系统级锁竞争/IO
...
fun RecyclerView.applyPerformanceOptimizations() {
    ...
    // 6. 注意：setDrawingCacheEnabled 在 API 28+ 已废弃且无效
    // 硬件加速下系统自动管理渲染缓存，无需手动开启
}
