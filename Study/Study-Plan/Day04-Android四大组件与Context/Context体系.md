...
Context 的核心设计目标是：**提供统一的应用运行环境访问接口**。

**关键设计约束**：
1. **统一接口**：不同组件（Activity、Service、Application）使用相同的 API
2. **环境隔离**：不同组件有不同的生命周期和能力
3. **资源管理**：统一管理应用资源、系统服务、文件系统
4. **安全控制**：通过 Context 限制组件的能力
...
**核心设计思想**：
- **接口抽象**：定义统一的 Context 接口，屏蔽实现细节
- **装饰器模式**：ContextWrapper 包装 ContextImpl，允许子类扩展功能
...
│                                                             │
│   ┌─────────────────┐                                       │
│   │     Context     │  (抽象类，定义核心 API)                │
│   └────────┬────────┘                                       │
│            │ extends                                        │
│            ▼                                                │
│   ┌─────────────────┐                                       │
│   │  ContextWrapper │  (装饰器模式，持有 mBase)              │
│   │                 │                                       │
│   │ - 持有 mBase    │                                       │
│   │ - 委托给 mBase  │                                       │
│   └────────┬────────┘                                       │
...
│   ├─────────────────┼──────────────┼──────────────┼──────────────┤  │
│   │ 启动 Activity   │ ✅ 需NEW_TASK│  ✅ 不需要   │ ✅ 需NEW_TASK│  │
│   │ 显示 Dialog     │ ❌ 不支持    │  ✅ 支持     │ ❌ 不支持    │  │
│   │ 启动 Service    │ ✅ 支持      │  ✅ 支持     │ ✅ 支持      │  │
│   │ 发送 Broadcast  │ ✅ 支持      │  ✅ 支持     │ ✅ 支持      │  │
│   │ 创建 View       │ ❌ 无主题    │  ✅ 有主题   │ ❌ 无主题    │  │
│   └─────────────────┴──────────────┴──────────────┴──────────────┘  │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
### 3.1 Context 继承关系：装饰器模式的应用

**设计思路**：
Context 体系使用**装饰器模式**，通过 ContextWrapper 包装真正的实现类 ContextImpl，实现以下目标：
1. **接口统一**：所有组件通过相同的 Context 接口访问功能
2. **能力扩展**：子类可以重写特定方法，添加额外功能（如主题）
3. **实现分离**：接口定义与实现分离，便于维护和扩展
...
1. **灵活性**：可以在运行时动态添加功能
2. **可扩展性**：子类可以重写特定方法
3. **职责分离**：ContextWrapper 负责委托，ContextImpl 负责实现

### 3.2 Context 数量计算

**设计思路**：
Context 的数量与组件实例一一对应，理解数量计算有助于避免内存泄漏。
...
**示例**：
- 1 个 Application + 3 个 Activity + 2 个 Service = 6 个 Context
- 每个 Activity/Service 实例都有独立的 Context
- Application Context 是全局单例
